function write_ply(vertex,face,filename,C,mode)
...
if (length(C)>0)
    Data.vertex.red = C(:,1);
    Data.vertex.green = C(:,2);
    Data.vertex.blue = C(:,3);
end
...
for i = 1:NumElements
   ...
   for j = 1:length(PropertyNames{i})
      ...
      if Type{i}(j) <= 8
         ...
         fprintf(fid,'property list uchar %s %s\n',PlyTypeNames{Type{i}(j)-7},PropertyNames{i}{j});%pcl only works with int
      end
   end
end
