function matches_after = RPFM(model,part,main_params,sparsecorrs)
             diam=sqrt(sum(calc_tri_areas(model)));

            if ~isfield(main_params,'num_deltas'), main_params.num_deltas = 1000;end;%round(part.n/10);end;
            if ~isfield(main_params,'delta_radius'),main_params.delta_radius = 1/sqrt(main_params.num_deltas)*diam;end;
    ...
    if exist('sparsecorrs'),
        if ~isfield(main_params,'init_delta_radius'),main_params.init_delta_radius = 1/sqrt(size(sparsecorrs,1))*diam;end;

        part_.TRIV = part.TRIV; part_.VERT = [part.VERT];
        model_.TRIV = model.TRIV; model_.VERT = [model.VERT];
        pts = sparsecorrs(:,1);
        FG = compute_indicator_functions({part_,model_}, [pts sparsecorrs(:,2)]', main_params.init_delta_radius);
    ...
    for reiterate = 1:main_params.num_reiterate+1
        ...
        if ~isfield(main_params,'icp_maxiter'), params.icp_maxiter = 30; else, params.icp_maxiter = main_params.icp_maxiter; end;
        ...
        if params.is_plot ,figure(6), end
        ...
        if main_params.verbose
            %figure(3),imagesc(C_out);colorbar;title('After diagonalization');caxis([-1 1]);axis image
        end
        ...
        uu_new = part.evecs_new'*part.S*ones(part.n,1);
        if main_params.verbose
        ...
        if reiterate~=main_params.num_reiterate+1
            ...
            if main_params.verbose
                ind = zeros(part_.n,1);ind(fps)=1;figure(4),showshape(part_,ind);caxis([0 1])
                ind = zeros(model_.n,1);ind(matches_after(fps))=1;figure(5),showshape(model_,ind);caxis([0 1])
            end
