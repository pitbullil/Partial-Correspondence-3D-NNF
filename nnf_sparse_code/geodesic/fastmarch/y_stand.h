...
struct Triangle { 	/* defines one triangle					*/
	...
	float  b[5];   /* surface cooefficients				*/
...
struct Stencil {		/* virtual numerical connections  */
	...
	float	Ctheta;		/* cos(theta) angle between the edges */
	float  Stheta; /* sin(theta)^2 when theta is the angle between the edges */
	int	v1,v2;		/* index of neigboring vertices */
	float	l1,l2;		/* edge length to v1 and to v2 */
	float Ctheta_mul_l1_div_l2_minus_one_mul_2, Ctheta_mul_l2_div_l1_minus_one_mul_2;
								/* Remember the value of the l1 and l2 multiplies
										    by Ctheta minus one	multiplied by 2	*/
	float sqr_l1, sqr_l2; /* The values of l1 and l2 multiplied by themselves. */
						/* Replaces 1 - a2 * (ctheta_mul_mb_div_ma_minus_one_mul_2 + 1) / b2 */ 
	float shortcut1_1, shortcut1_2;
						/* Replaces - Stheta * a2*/ 
	float shortcut2_1, shortcut2_2;
...
struct Point {						/* defines a point in 3D with a U value					*/
	float  x,y,z;					/* x,y,z coordinates IN 3D								*/
	float  U;						/* The U value of the point.							*/
};

struct Vector {						/* defines a vector in 3D								*/
		float  x,y,z;				/* x,y,z coordinates of the vector in 3D.				*/
};
