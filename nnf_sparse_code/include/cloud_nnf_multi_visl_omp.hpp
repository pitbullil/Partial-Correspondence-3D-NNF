...
public:
	...
	int candidates = 5;
	bool gopt_use_h = false, gopt_use_m = false, gopt_use_l = true;
	int num_samples,model_vertices,part_vertices;
	float R_thresh, R_thresh_ms, R_thresh_ls,density = 0.05;
	...
	int gopt_hl_cands = 8;//how many candidates we will look for in greedy opt
	int gopt_ml_cands = 4;//how many candidates we will look for in greedy opt
	int gopt_ll_cands = 6;//how many candidates we will look for in greedy opt
	...
	void visualizeDDIS(uint32_t src, uint32_t target, float multiplier = 1);
	void visualizeWDIS(uint32_t src, uint32_t target, float multiplier = 1);
...
template<class CloudT, class D> vector<vector<ind_dis_s>> Feature1<CloudT,D>::calculateSimilarity() {
	...
	for (uint32_t j = 0; j < num_samples; j++) {
		...
		if (P.r_thresh < 100) { R = min(R,R_thresh); };
		TRadiuses.push(ind_r_s(j,R*1.10));
	}
...
#pragma omp parallel for
	for (int t = 0; t < P.threads; t++) {//calculating similarity for each search window
		...
		for (int i = thread_start.at(t); i < thread_end.at(t); i++) {//calculating similarity for each search window
			pointSimilarity(i, TDistances, F_R, t);//sim holds the index of the point in Q and it's score
			...
			switch (P.similarity){
				...
				case TRIDISP: {
					Result.at(feature_point_inds.at(i)).at(0) = lower_scale_sim_max(feature_point_inds.at(i), Result.at(feature_point_inds.at(i)).at(0).first, i, MSWDIS);//finding lower scale similarity
																																										  //if (j == 0) 
					Mmax.at(feature_point_inds.at(i)) = Result.at(feature_point_inds.at(i)).at(0).second;
					Result.at(feature_point_inds.at(i)).at(0) = lower_scale_sim_max(feature_point_inds.at(i), Result.at(feature_point_inds.at(i)).at(0).first, i, TRIDISP);//finding lower scale similarity
																																										  //if (j == 0) 
					Lmax.at(feature_point_inds.at(i)) = Result.at(feature_point_inds.at(i)).at(0).second; break;
				}
...
template<class CloudT, class D> void Feature1<CloudT, D>::pointSimilarity(uint32_t i, vector<vector<float>>& TDistances, vector<ind_r_s>& F_R,int t) {
	...
	extractTemplateCandidate(i, scene_P_D_t.at(t), extract,R,M);//to be done for 1st feature point and trimmed down with each smaller radius point
